...
const App = () => {
  ...
  // Function to clear input and focus back to new data
  const clearInput = () => {
    setNewName('')
    setNewNumber('')
  ...
  // Fetch persons array from server once rendered for first time
  useEffect(() => {
  ...
  // Adding a person to phonebook
  const addPerson = event => {
    event.preventDefault()

    const existingPerson = persons.find(person => person.name === newName);

    // Update person if already exists
    if (existingPerson) {
